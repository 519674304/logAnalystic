...
- 所有 stage 的 `start_matcher_ids` / `end_matcher_ids` 都支持数组（**按数组顺序优先**，首个命中的决定起止）；`start_matcher_id` / `end_matcher_id` 是单元素简写。拦截 stage 用它覆盖全部拦截结束事件（拦截仍是**任一命中**即丢弃整请求）。
...
- `stage.start_matcher_ids` / `stage.end_matcher_ids`（数组）可用于任意 stage（**按数组顺序优先**，首个命中的决定起止），每个引用都必须指向启用的 matcher；`start_matcher_id` / `end_matcher_id` 是单元素简写。
