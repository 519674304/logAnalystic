...
- `rule-package.zip`：时延分析冒烟规则包（3 层 TOML，对齐 `fixture/sample-latency.log` 的消息与 4 应用）。
- `rule-package/`：上面的 4 个 TOML 源文件（definitions/matchers/stages + manifest，修改后重新打包）。
...
5. 切到时延分析页，点「分析」。
6. 检查：请求资源管理器显示 5 个请求（含 1 个慢请求 375ms）；泳道图显示真实阶段时延；区间统计显示 25 个样本的 avg / P90 / max。
